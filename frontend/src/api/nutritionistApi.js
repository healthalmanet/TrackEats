...
// Axios instance for nutritionist-scoped endpoints
const axiosInstance = axios.create({
  baseURL: "https://trackeats.onrender.com/api/nutritionist",
  headers: {
    "Content-Type": "application/json",
...
// --- CORRECTED Lab Report Functions using the Nutritionist-scoped endpoint ---

/**
 * Fetches all lab reports for a specific patient assigned to the nutritionist.
 * @param {string} patientId The ID of the patient.
 */
export const getAllLabReports = async (patientId) => {
  await delay(500);
  // Uses the correct endpoint: /api/nutritionist/patients/:id/lab-reports/
  return axiosInstance.get(`/patients/${patientId}/lab-reports/`);
};

/**
 * Fetches a single lab report for a patient by its specific date.
 * @param {string} patientId The ID of the patient.
...
// --- Axios instance for endpoints NOT under /nutritionist ---
const generalAxios = axios.create({
  baseURL: "https://trackeats.onrender.com/api",
  headers: {
    "Content-Type": "application/json",
...
// --- Generate Diet Plan (uses general /api base) ---
export const generateDietPlan = async (patientId) => {
  await delay(500);
  return generalAxios.post(`/patients/${patientId}/generate-plan/`);
};
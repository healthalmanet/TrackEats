...
const QuickMealLogger = ({ onMealLogged }) => {
  const {
    ...
    mealType,
    setMealType,
    logDate, // Assuming state for date is provided by the hook
    setLogDate, // Assuming setter for date is provided by the hook
    logTime, // Assuming state for time is provided by the hook
    setLogTime, // Assuming setter for time is provided by the hook
    handleSubmit,
    loggedMeals,
    handleDeleteMeal,
    pagination,
  ...
  const mealTypeStyles = {
    "early-morning": {
      border: "border-[var(--color-accent-1-text)]",
      bg: "bg-[var(--color-accent-1-bg-subtle)]",
      iconColor: "text-[var(--color-accent-1-text)]",
    },
    breakfast: {
    ...
    "mid-morning snack": {
      border: "border-[var(--color-accent-2-text)]",
      bg: "bg-[var(--color-accent-2-bg-subtle)]",
      iconColor: "text-[var(--color-accent-2-text)]",
    },
    lunch: {
    ...
    "afternoon snack": {
      border: "border-[var(--color-accent-3-text)]",
      bg: "bg-[var(--color-accent-3-bg-subtle)]",
      iconColor: "text-[var(--color-accent-3-text)]",
    },
    dinner: {
  ...
  return (
    <section className="w-full bg-[var(--color-bg-app)] px-6 sm:px-12 py-16 font-[var(--font-secondary)]">
      <div className="max-w-7xl mx-auto">
        ...
        >
          <h2 className="text-center text-2xl sm:text-3xl font-[var(--font-primary)] font-bold text-[var(--color-text-strong)]">
            Quick Meal Logger
          </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          >
                ...
                >
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    <input
                      ...
                      placeholder="Qty"
                      className="w-20 bg-[var(--color-bg-app)] text-[var(--color-text-strong)] border-2 border-[var(--color-border-default)] rounded-lg px-2 py-2 text-sm placeholder:text-[var(--color-text-muted)] focus:outline-none focus:border-[var(--color-primary)] transition"
                    />
                    <select
                      value={item.unit}
                      onChange={(e) =>
                      ...
                      className="bg-[var(--color-bg-app)] text-[var(--color-text-strong)] border-2 border-[var(--color-border-default)] rounded-lg px-2 py-2 text-sm focus:outline-none focus:border-[var(--color-primary)] transition"
                    >
                  ...
                  <input
                    ...
                    value={item.remark}
                    onChange={(e) =>
                      handleFoodChange(index, "remark", e.target.value)
                    }
                    placeholder="Remark (optional)"
                    className="w-full bg-[var(--color-bg-app)] border-2 border-[var(--color-border-default)] text-[var(--color-text-strong)] rounded-lg px-3 py-2 text-sm placeholder:text-[var(--color-text-muted)] focus:outline-none focus:border-[var(--color-primary)] transition"
                  />
                </motion.div>
              ))}
            </AnimatePresence>
            ...
            <div className="flex gap-4 mb-6">
              <button
                ...
                className="text-[var(--color-primary)] text-sm font-semibold hover:text-[var(--color-primary-hover)] transition-colors flex items-center gap-1"
              >
              ...
              {foodInputs.length > 1 && (
                <button
                  ...
                  className="text-[var(--color-danger-text)] text-sm font-semibold hover:opacity-80 transition-colors flex items-center gap-1"
                >
            ...
            {/* START: Added Date and Time fields */}
            <div className="mb-6">
              <label className="block text-base mb-2 font-medium text-[var(--color-text-strong)]">
            ...
            {/* END: Added Date and Time fields */}
                  ...
                  <label
                    ...
                    className={`px-4 py-2 rounded-full text-sm font-semibold cursor-pointer border-2 transition-all duration-200 ${
                      mealType === value
                        ...
                        : "border-[var(--color-border-default)] text-[var(--color-text-default)] bg-[var(--color-bg-surface)] hover:border-[var(--color-primary)] hover:text-[var(--color-text-strong)]"
                    }`}
                  >
                    <input
                      type="radio"
            ...
            <div className="flex justify-center">
              <button
                ...
                className="w-full sm:w-3/5 bg-[var(--color-primary)] text-[var(--color-text-on-primary)] hover:bg-[var(--color-primary-hover)] px-6 py-3 rounded-full text-lg font-bold font-[var(--font-primary)] transition-all duration-300 shadow-lg hover:shadow-xl hover:-translate-y-1 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none"
              >
                {isSubmitting ? (
          ...
          <motion.div
            ...
            className="bg-[var(--color-bg-surface)] rounded-2xl p-6 shadow-xl border-2 border-[var(--color-border-default)]"
          >
            <h3 className="text-xl font-semibold mb-4 text-[var(--color-text-strong)] font-[var(--font-primary)]">
              Logged Meals
            </h3>
                ...
                {pagination.count > 5 && (
                  <div className="flex justify-center items-center mt-6 space-x-2">
                    ...
                    <span className="text-sm text-[var(--color-text-default)] font-semibold">
                      Page {currentPage} of {Math.ceil(pagination.count / 5)}
                    </span>
                    <button

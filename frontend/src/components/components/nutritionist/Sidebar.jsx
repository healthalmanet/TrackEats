...
const Sidebar = () => {
  ...
  const linkClasses = ({ isActive }) =>
    `flex items-center gap-3 pl-5 py-2 rounded-lg transition-all duration-200 hover:bg-green-100 hover:text-green-700 ${
      isActive ? "bg-green-100 text-green-700 font-medium border-l-4 border-green-500" : "text-gray-700"
    }`;

  return (
    <>
      {/* Toggle button for small screens */}
      <div className="md:hidden flex items-center p-4 bg-white shadow-md w-full">
        <button
      ...
      <aside
        className={`$ {
          isOpen ? "block" : "hidden"
        } md:flex w-60 h-screen bg-gray-50 shadow-md p-0 flex-col items-start fixed md:static z-50`}
      >
        <nav className="w-full mt-10 px-4">

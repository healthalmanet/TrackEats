...
const Meals = () => {
  ...
  const fetchMeals = async () => {
    try {
      ...
      // Only transform meals if they exist
      if (response.meals) {
        const transformedMeals = Object.entries(response.meals).map(([date, data]) => ({
          id: date,
          date: date,
  ...
  // **NEW**: Handle the case where the diet is not approved
  // We assume the successful status is exactly 'approved'. Any other status will show this message.
  ...
  // The rest of the component will only render if the diet IS approved.
  return (
    <>
      <div className="min-h-screen bg-white p-6 font-sans text-[#001064]">
        {/* Header */}
